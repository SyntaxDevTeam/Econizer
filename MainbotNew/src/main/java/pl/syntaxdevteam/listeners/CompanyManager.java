...
public class CompanyManager extends ListenerAdapter {
            ...
            try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                ...
                if (rs.next()) {
                    ...
                    if (owner.equals(userId)) {
                        ...
                        event.replyEmbeds(eb.build()).addActionRow(Button.danger("comp_leave", "🚪 Opuść firmę")).queue();
                    }
                } else {
    ...
    public void onButtonInteraction(@NotNull ButtonInteractionEvent event) {
        ...
        if (action.equals("comp_create_cash") || action.equals("comp_create_card")) {
            TextInput nameInput = TextInput.create("company_name", "Nazwa Przedsiębiorstwa", TextInputStyle.SHORT).setMinLength(3).setMaxLength(30).build();
            Modal modal = Modal.create("modal_" + action, "Rejestracja działalności").addActionRow(nameInput).build();
